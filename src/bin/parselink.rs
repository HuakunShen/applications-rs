...
fn main() {
    ...
    let lnk_path = PathBuf::from("C:\\ProgramData\\Microsoft\\Windows\\Start Menu\\Docker Desktop.lnk");
    ...
    let lnk_path = PathBuf::from("C:\\Users\\shenh\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Lark.lnk");
    ...
    let abs_path = lnk_path.clone().parent().unwrap().join(lnk.string_data.relative_path.unwrap());
